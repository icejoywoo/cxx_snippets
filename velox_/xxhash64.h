...
// https://raw.githubusercontent.com/stbrumme/xxhash/master/xxhash64.h
...
public:
    ...
    /// refer: io.airlift.slice.hash(long value)
    static uint64_t hash(long value)
    {
        uint64_t hash = Prime5 + 8;
    ...
    static uint64_t mix(uint64_t current, uint64_t value)
    {
        return rotateLeft(current + value * Prime2, 31) * Prime1;
    }

    static uint64_t finalShuffle(uint64_t hash)
    {
        hash ^= hash >> 33;
        hash *= Prime2;
        hash ^= hash >> 29;
